...
void LPS25HB_wb(uint8_t reg_add, uint8_t* val, size_t len) {
    i2c_master_write_multi(val, len, reg_add, LPS25HB_address, 0);
}

void LPS25HB_rb(uint8_t reg_add, uint8_t* val, size_t len) {
    i2c_master_read(val, len, reg_add, LPS25HB_address, 1);
}
...
void LPS25HB_press(float* pressure) {
    uint8_t buffer[3];

    LPS25HB_rb(LPS25HB_PRESSURE_OUT | 0x80, buffer, 3);
